...
namespace Art.Shaders
{
    ...
    {
        OverlayGradientPostProcessPass pass;

        public override void Create()
        {
            pass = new OverlayGradientPostProcessPass();
        }
    ...
    [System.Serializable]
    public class OverlayGradientPostProcessPass : ScriptableRenderPass
    {
        ...
        public OverlayGradientPostProcessPass()
        {
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            var materials = OverlayGradientPostProcessingMaterials.Instance;
            if (materials == null)
            {
            ...
            //---Custom effect here---
            var customEffect = stack.GetComponent<OverlayGradientEffect>();
            // Only process if the effect is active
            if (customEffect.IsActive())
            {
                var material = materials.material;
                // P.s. optimize by caching the property ID somewhere else
                material.SetFloat(Shader.PropertyToID("_Radius"), customEffect.radius.value);
                material.SetFloat(Shader.PropertyToID("_Feather"), customEffect.feather.value);
                
                material.SetColor(Shader.PropertyToID("_tlColor"), customEffect.topLeftColor.value);
                material.SetColor(Shader.PropertyToID("_trColor"), customEffect.topRightColor.value);
                material.SetColor(Shader.PropertyToID("_blColor"), customEffect.bottomLeftColor.value);
                material.SetColor(Shader.PropertyToID("_brColor"), customEffect.bottomRightColor.value);
